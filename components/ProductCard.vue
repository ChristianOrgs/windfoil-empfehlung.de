<template>
  <article class="bg-white rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col">
    <!-- Image -->
    <NuxtLink :to="`/produkte/${product.slug}/`" class="aspect-square bg-white relative block overflow-hidden group border-b border-border">
      <img
        v-if="product.image"
        :src="product.image"
        ...
        height="400"
      />
      <div v-else class="absolute inset-0 flex flex-col items-center gap-2 text-muted p-6 text-center bg-sky justify-center">
      ...
      <!-- Sale Badge -->
      <div v-if="salePct" class="absolute top-3 right-3">
    ...
    <div class="p-5 flex flex-col flex-1 gap-3">
      <div>
        ...
        <NuxtLink :to="`/produkte/${product.slug}/`" class="hover:text-ocean transition-colors">
...
import { formatSavings, isPriceStale } from '~/utils/price'

const props = defineProps<{ product: WingfoilProduct }>()
const config = useRuntimeConfig()
const primaryOffer = computed(() => getPrimaryOffer(props.product))

const salePct = computed(() => {
  const offer = primaryOffer.value
  if (!offer || isPriceStale(offer.lastChecked) || !offer.currentPrice || !offer.originalPrice) return null
  if (offer.originalPrice <= offer.currentPrice) return null
  return formatSavings(offer.currentPrice, offer.originalPrice).savingsPct
})
